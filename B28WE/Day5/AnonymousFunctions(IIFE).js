...
let convertUppercase = function (stringArray) {
  let res = stringArray.map((name) => name[0].toUpperCase() + name.slice(1));
  console.log(res);
};
...
let rotateArray = function (arr, k) {
  let temp = [];
  for (let i = 0; i < k; i++) temp.push(arr[i]);

  for (let j = 0; j < k; j++) arr.shift();
